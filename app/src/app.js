...
// Set app name early for dev mode (Dock, about panel)
try {
  app.setName("LARSA TIME");
  if (process.platform === "darwin" && app.setAboutPanelOptions) {
    app.setAboutPanelOptions({
      applicationName: "LARSA TIME",
    });
  }
} catch (_) {}
...
app.once("ready", async () => {
  ...
  window.webContents.session.webRequest.onHeadersReceived(
    (details, callback) => {
      ...
      // Apply assets policy
      cspValue +=
        "style-src 'self' data: 'unsafe-inline'; font-src 'self' data:; img-src 'self' data: file:;";
  ...
  // Hide menu bar, keep Alt to toggle
