...
tray.setToolTip(translation.translate("Cattr"));
...
const trayMenuElements = [
  ...
  },
  {
    ...
    visible: false,
    click: () => {
      if (!tracker.active && selectedTaskId !== null) tracker.start();
    },
  },
  {
    ...
    visible: false,
    click: () => {
      if (tracker.active && selectedTaskId !== null) tracker.stop();
    },
  },
  {
...
tray.on("click", (e) => {
  ...
  if (e.ctrlKey && selectedTaskId !== null) {
    ...
    else tracker.start(selectedTaskId);
...
const onStartTrack = (taskId) => {
  ...
  if (Object.prototype.hasOwnProperty.call(availableTasks, taskId))
    trayMenu.items[MENU_ITEMS.TASKS].submenu.items[
...
taskController.events.on("pulled", (tasksReference) => {
  ...
  // Dereference tasks
  else tasks = tasksReference.slice();
  ...
  tasks.forEach((task, index) => {
    trayMenuElements[MENU_ITEMS.TASKS].submenu.push({
      id: task.id,
      label:
        task.name.substr(0, MAX_TASK_TEXT_LENGTH) +
        (task.name.length > MAX_TASK_TEXT_LENGTH ? "..." : ""),
      type: "radio",
      click: () => {
        if (selectedTaskId !== task.id) tracker.start(task.id);
      },
    });
  ...
  trayMenuElements[MENU_ITEMS.TASKS + 1].visible =
    trayMenuElements[MENU_ITEMS.TASKS].visible;

  tray.setContextMenu((trayMenu = buildMenuWithTranslations(trayMenuElements)));

  if (tracker.active && selectedTaskId !== null) onStartTrack(selectedTaskId);
  else if (!tracker.active && selectedTaskId !== null) onStopTrack();
});
...
translation.on("language-changed", () => {
  ...
  trayMenu.items[MENU_ITEMS.TASKS].submenu.items[0].visible =
    selectedTaskId === null;

  if (
    selectedTaskId &&
    Object.prototype.hasOwnProperty.call(availableTasks, selectedTaskId)
  )
    trayMenu.items[MENU_ITEMS.TASKS].submenu.items[
