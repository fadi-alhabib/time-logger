...
class TaskTracker extends EventEmitter {
  constructor() {
    ...
    this.on("activity-proof-result", async (result) => {
      ...
      log.debug(
        "Activity is not confirmed via interactive popup, stopping tracker"
      );
    ...
    this.on("interval-removed", async (interval) => {
      ...
      await TimeController.saveTrackedTime(
        interval.task.id,
        interval.duration,
        "subtract"
      );
    ...
    this.on("interval-capture", async () => {
      ...
      this.captureCurrentInterval(
        ticks,
        this.currentInterval.startedAt,
        new Date()
      );
  ...
  startInactivityDetection() {
    ...
    if (this.inactivityDetectionTimerId)
      throw new UIError(
        500,
        "Reject request to spawn overlapping inactivity detector"
      );
  ...
  stopInactivityDetection() {
    ...
    if (!this.inactivityDetectionTimerId) return false;
  ...
  async pauseTicker() {
    // Pausing ticker
    if (!this.ticker.paused) this.ticker.pause();

    // Stopping inactivity detector
    this.stopInactivityDetection();

    // Mark interval as interrupted
    if (this.active) this.currentInterval.everPaused = true;
  }
  ...
  async resumeTicker() {
    // Starting tracker
    if (this.ticker.paused) this.ticker.resume();
  ...
  async start(taskId) {
    ...
    if (typeof taskId !== "string") {
      ...
      if (!this.previousTask)
        throw new UIError(
          500,
          "Tracker re-start request rejected due to lack of previous task"
        );
      ...
      if (!task)
        throw new UIError(
          500,
          `Tracker start request rejected due to inability to find a task with id: ${taskId}`
        );
    ...
    this.currentInterval.everPaused = false;
    this.currentTaskTimeTrackedToday = await TaskController.getTaskTodayTime(
      this.currentTask.id
    );
    ...
    // Enable heartbeat
    if (!OfflineMode.enabled) heartbeatMonitor.start();
    else log.debug("Skipping heartbeat enabling, since we're offline");
    ...
    if (trackingFeatures.includes("APP_MONITORING")) activeWindow.start();

    // Dispatch corresponding event
    this.setTrackerStatus(true);
    this.emit(action, this.currentTask.id);
    log.debug(
      `Started task "${this.currentTask.name}" with ${this.captureInterval}s capture interval`
    );
  }
  ...
  async stop(pushInterval = true, emitEvent = true) {
    ...
    if (emitEvent) this.emit("stopping");

    // Stop active window tracking if active
    if (activeWindow.active) activeWindow.stop();
    ...
    log.debug(
      `Executing tracker stop request (push = ${pushInterval}, dispatch = ${emitEvent})`
    );

    // Capturing & pushing interval, if it is longer than one second
    if (pushInterval && ticks >= 1) await this.captureCurrentInterval(ticks);
  ...
  async captureCurrentInterval(ticksOverride, startAtRaw, endAtRaw) {
    ...
    if (!this.active || !this.currentTask)
      throw new UIError(
        500,
        "Rejected interval capture due to stopped tracker"
      );
    ...
    try {
      ...
      const currentTrackingFeatures =
        (await trackingFeature.getCurrentFeatures()) || [];

      // Get amount of ticks tracked
      const ticks =
        typeof ticksOverride === "number" ? ticksOverride : this.ticker.ticks;
      ...
      if (
        ...
        typeof startAt.getTime !== "function" ||
      ...
      log.debug(
        `Capturing interval: ${startAt} ~ ${endAt} (duration = ${ticks})`
      );
      ...
      const mustCapture =
        this.currentTask?.Project?.screenshotsState ===
        ScreenshotsState.REQUIRED;
      const optionalCapture =
        this.currentTask?.Project?.screenshotsState ===
        ScreenshotsState.OPTIONAL;
      if (
        mustCapture ||
        (optionalCapture &&
          currentTrackingFeatures.includes("DESKTOP_SCREENSHOTS")) ||
        (this.currentTask?.Project == null &&
          currentTrackingFeatures.includes("DESKTOP_SCREENSHOTS"))
      ) {
        try {
          ...
          log.error(
            "Error occured during screenshot capture for interval",
            err,
            true
          );
      ...
      // Pushing interval
      try {
        const pushedInterval = await IntervalsController.pushTimeInterval(
          interval,
          intervalScreenshot
        );
        ...
        if (error.request) {
          ...
          await IntervalsController.backupInterval(
            interval,
            intervalScreenshot
          );
        } else {
      ...
      throw new UIError(
        500,
        `Unhandled system error occured: ${error}`,
        "EAUTH502",
        error
      );
    }
