 ...
 * @type {String}
 */
const packageId = isDeveloperModeEnabled ? "larsa-time-develop" : "larsa-time";
...
const configuration = {
  ...
  appdata: isDeveloperModeEnabled
    ? app.getPath("userData").concat("-develop")
...
configuration.sentry = {
  ...
  dsnFrontend:
    "https://00bd1ee1db824310812252bb96e96945@sentry.amazingcat.net/14",

  // Setting the current release
  release: `larsa-time@${packageVersion}`,
};

// Ensure that application data directory actually exists
if (!fs.existsSync(configuration.appdata)) fs.mkdirSync(configuration.appdata);
...
configuration.updateNotification = {
   ...
   */
  manifestBaseUrl:
    "https://git.amazingcat.net/api/v4/projects/353/releases/permalink/latest",
