<template>
  <div class="application">
    <router-view />
    <canvas ref="screen-capture" hidden />
  </div>
</template>
...
export default {
  ...
  async mounted() {
    ...
    if (this.$i18n.locale === "ru") document.title = this.$t("Cattr");
  ...
  methods: {
    ...
    updateTrackingFeatures: debounce(async function (features = null) {
      ...
      try {
        ...
        if (req.code !== 200) throw new Error(req.body.message);
    ...
    updateNotSyncedScreenshotsAmount: debounce(async function () {
      try {
        ...
        const req = await this.$ipc.request(
          "interval/not-synced-screenshots-amount",
          {}
        );
    ...
    displayTrackingFeatures: debounce(async function (
      features,
      updated = false
    ) {
