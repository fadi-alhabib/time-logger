<template>
  <el-row ref="task" class="task" type="flex">
    <el-col :span="18" class="task-info">
      <p class="task-name clickable" @click="openTask">
        {{ task.name }}
      </p>
      <p class="project-name clickable" @click="openProject">
        <i class="el-icon-camera" :class="screenshotIconClass"></i>
        {{ projectName }}
      </p>
    </el-col>
    <el-col :span="2" class="task-controls pin">
      <el-button
    ...
    <el-col :span="4" class="task-controls">
  ...
  methods: {
    ...
    openTask() {
      ...
      if (
        ...
        this.$route.params.id === this.task.id
      )
        return;
    ...
    openProject() {
      ...
      if (
        ...
        this.$route.params.id === this.task.projectId
      )
        return;
    ...
    async pinner() {
      ...
      if (this.isPinned) this.$store.dispatch("pinTask", { id: this.task.id });
      else this.$store.dispatch("unpinTask", this.task.id);
    ...
    async track() {
      ...
      if (this.clickProtected || this.loading) return;
