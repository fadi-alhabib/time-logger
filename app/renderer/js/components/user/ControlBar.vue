<template>
  <el-container class="control-bar">
    ...
    <el-button
      ...
      @click="sync"
    >
      <i class="el-icon-refresh" :class="{ animated: syncInProgress }" />
    </el-button>
    <el-button
      v-if="!searchFieldActive && $route.path === '/user/tasks'"
...
export default {
  ...
  methods: {
    ...
    async returnEmptyError() {
      ...
      this.$alert(
        this.$t(
          "Your daily report is empty. Get back and do some work before!"
        ),
        this.$t("Daily report"),
        {
    ...
    async getReport() {
      ...
      if (reportBuffer !== "") {
        this.$confirm(
          this.$t(
            "Your daily report is ready. In which format you'd like to export it?"
          ),
          this.$t("Daily report"),
          {
