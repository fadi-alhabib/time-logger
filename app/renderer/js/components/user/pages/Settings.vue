<template>
  <el-container class="settings-container">
    ...
    <el-form
      ref="settings"
      class="settings-form elevated glass"
      label-position="top"
    >
      <el-form-item
      ...
      >
        <template
          v-if="
            formField.frontend.type === 'number' ||
            formField.frontend.type === 'text'
          "
        >
          <el-input
            ...
            v-bind="{
              min: formField.frontend.options.min,
              max: formField.frontend.options.max,
            }"
        ...
        <small class="el-form-item-comment">{{
          $t(formField.description)
        }}</small>
      </el-form-item>
    </el-form>
    <div class="settings__footer">
      <el-button :loading="savingInProgress" type="primary" @click="onSubmit">
        {{ $t("Save") }}
      </el-button>
      <el-button
      ...
      <br />
      <br />
      <el-button :loading="ssoLoading" type="secondary" @click="performSSO">
        {{ $t("Sign into web app") }}
      </el-button>
      <br /><br />
      <small class="version-text">
        LARSA Logger {{ version.number }} 💖
        <br />
        {{ version.devMode ? "+dev " : "" }}
...
export default {
  ...
  async mounted() {
    this.$ipc
      .request("user-preferences/export-structure", {})
      .then(({ body }) => {
        ...
        Object.entries(body.preferences).forEach(([prefKey, prefVal]) => {
          const renderableField = {
            ...
            label: prefVal.name,
            value:
              typeof prefVal.value !== "undefined" && prefVal.value !== null
                ? prefVal.value
                : prefVal.default,
            description: prefVal.description,
            frontend: {
              type: ["toggle", "options"].includes(prefVal.frontend.element)
                ? "select"
                : prefVal.frontend.element,
            },
          };

          if (
            ["options", "select", "toggle"].includes(prefVal.frontend.element)
          ) {
            const opts = Object.entries(prefVal.frontend.options).map(
              ([key, value]) => ({ label: key, value })
            );

            renderableField.frontend.options = opts;
          } else renderableField.frontend.options = prefVal.frontend.options;
  ...
  methods: {
    ...
    async performSSO() {
      ...
      const req = await this.$ipc.request(
        "auth/request-single-click-redirection",
        {}
      );
      ...
      if (req.code !== 200)
        ...
        this.$message({
          type: "error",
          message: `${this.$t("Error")}: ${req.body.message}`,
        });
      // Show success message
      else
        this.$message({
          type: "success",
          message: this.$t("Success! Opening your browser..."),
        });
    ...
    async logout() {
      ...
      this.$store.dispatch("stopTrack", { $ipc: this.$ipc }).catch((error) => {
      ...
      if (req.code === 200) this.$store.dispatch("logout", this.$ipc);
      else {
