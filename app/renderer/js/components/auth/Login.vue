<template>
  <div class="login-container">
    <el-card ref="card" class="box-card login glass elevated">
      <div slot="header" class="clearfix">
        <img :src="'assets/bg.svg'" alt="LARSA" class="bg-image" />
...
export default {
  ...
  async mounted() {
    ...
    const ssoParamsRequest = await this.$ipc.request(
      "auth/check-sso-presence",
      {}
    );
    if (ssoParamsRequest.code === 200) this.promptForSSO(ssoParamsRequest.body);
  },
  ...
  methods: {
    async promptForSSO(ssoParams) {
      ...
      this.$confirm(
        this.$t("Are you sure you want to sign in to").concat(
          `: ${url.origin}?`
        ),
        ...
        .then(async () => {
          ...
          const authRequest = await this.$ipc.request(
            "auth/perform-sso",
            ssoParams
          );
    ...
    async checkHostname() {
      ...
      const res = await this.$ipc.request(ipcRoute, {
